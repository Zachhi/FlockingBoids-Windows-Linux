...
void Simulation::addNewBoid(int posx, int posy, float rot)
{
...
void Simulation::updateGameSpeed() //separate this from updateObjects() so we only have to check the if stmt once. This is bc we update boids every loop, but only update gameSpeed when it is changed
{
